        ...
        try:
            ...
            if result.success:
                ...
                if portfolio_return >= min_required:
                    logger.info(f"Min downside risk optimization successful with return constraint: "
                              f"expected_return={portfolio_return:.4f} >= {min_required:.4f}")
                    return weights
                else:
                    logger.warning(f"Return constraint not satisfied: {portfolio_return:.4f} < {min_required:.4f}")
                    # 제약 없이 재시도
                    return self._optimize_min_variance_without_return_constraint(cov_matrix)
            else:
                logger.warning("Min downside risk optimization with return constraint failed, trying without constraint")
                return self._optimize_min_variance_without_return_constraint(cov_matrix)
        except Exception as e:
            logger.error(f"Min downside risk optimization error: {e}, falling back to no-constraint version")
            return self._optimize_min_variance_without_return_constraint(cov_matrix)
